...
const int BUTTON1_sensorPin = 2;
const int BUTTON2_sensorPin = 3;
...
void setup() {
  ...
  //pins I was using for the ADDR and IRQ of the MPR on the earlier prototype
  pinMode(7, OUTPUT);
  // pinMode(8, OUTPUT);
  // digitalWrite(8, HIGH);
  // digitalWrite(7, HIGH);
  ...
  pinMode(BUTTON1_sensorPin, INPUT_PULLUP);
  pinMode(BUTTON2_sensorPin, INPUT_PULLUP);
...
void loop() {
  ...
  if (millis() >= last_interval_ms + INTERVAL_MS) {
    ...
    button1 = digitalRead(BUTTON1_sensorPin);
    button2 = digitalRead(BUTTON2_sensorPin);
  ...
  if (!button1) {
    ...
    //char ctrlKey = KEY_LEFT_GUI; //if compiling in windows
    // #define KEY_LEFT_CTRL 0x80
    // #define KEY_LEFT_GUI 0x83
    //     char ctrlKey = KEY_LEFT_GUI;

    //blehid.keyPress(ctrlKey);
#define KEY_CONTROL_RIGHT 0xE4
    blehid.keyPress(KEY_CONTROL_RIGHT);
    blehid.keyPress('z');
    delay(100);
    blehid.keyRelease();
    // delay(5);
    ...
    Serial.println("Sending a ctrl + Z");
    ...
    // delay(5);
    // blehid.mouseMove(0, MOVE_STEP);
    Serial.println("Now sending a right ctrl alone");

    // delay(1000);
    //     blehid.keyCommand(KEY_CONTROL_RIGHT);
    // //blehid.keyPress('z');
    // delay(100);
    // blehid.keyRelease();
    // // delay(5);
    ...
    // // delay(5);
    // // blehid.mouseMove(0, MOVE_STEP);
    // Serial.println("Now sending a right ctrl alone but with keyCommand");
  ...
  if (!button2) {
    ...
    delay(1000);  //do a click
    Serial.println("Click");
    blehid.mouseButtonRelease();
    blehid.mouseMove(50, 0);
    delay(500);  //movie in all four directions
    blehid.mouseMove(0, 50);
    delay(500);
    blehid.mouseMove(-50, 0);
    delay(500);
    blehid.mouseMove(0, -50);
    ...
    delay(5);
    blehid.keyRelease();
    Serial.println("Sending an password");
    delay(1000);
  }
